...
def texture_editing(prn, args):
    if args.mode == 3:
        with open(args.output_path, 'w') as f:
            json.dump(prn.triangles.tolist(), f)
        exit()
    ...
    image = image/255.
    texture = cv2.remap(image, pos[:, :, :2].astype(
        np.float32), None, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
    ...
    if Mode == 0:
        # load eye mask
        uv_face_eye = imread(
            'Data/uv-data/uv_face_eyes.png', as_grey=True)/255.
        ...
        ref_texture = cv2.remap(ref_image, ref_pos[:, :, :2].astype(
            np.float32), None, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
    ...
    # change whole face(face swap)
        ...
        ref_pos = prn.process(ref_image)
        ref_image = ref_image/255.
        ref_image2 = imread(args.ref_path2)
        ref_image2 = ref_image2/255.
        ref_texture = cv2.remap(ref_image2, ref_pos[:, :, :2].astype(
            np.float32), None, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
        # ref_vertices = prn.get_vertices(ref_pos)
        new_texture = (texture + ref_texture)/2.
    ...
    face_mask = render_texture(
        vertices.T, vis_colors.T, prn.triangles.T, h, w, c=1)
    face_mask = np.squeeze(face_mask > 0).astype(np.float32)

    new_colors = prn.get_colors_from_texture(new_texture)
    new_image = render_texture(
        vertices.T, new_colors.T, prn.triangles.T, h, w, c=3)
    ...
    # save output
    imsave(args.output_path, new_image)
    print('Done.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r2', '--ref_path2', default='TestImages/trump.jpg', type=str,
                        help='path to reference image2(texture ref)')
